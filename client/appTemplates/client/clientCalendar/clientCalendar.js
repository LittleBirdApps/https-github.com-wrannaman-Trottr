...
Template.clientCalendar.rendered = function () {
    Meteor.call('RemoveOldAvailability', function (error, result) {});
    if (!Meteor.user().walkerId) {
        Router.go('/findWalker');
        sweetAlert({
                title: "Uh Oh!",
                type: "error",
                text: "Please select a walker first",
                timer: 2000
            });

    }
    Session.set("Navbar", "Calendar");
    ...
    var myWalker = Meteor.user().walkerId;
    Meteor.call("CalEventsForMyWalker", myWalker, function (error,results) {
        if (error) {
            return sweetAlert({
                title: "Uh Oh!",
                type: "error",
           ...
           return Session.set("CalEventsForMyWalker", results);
         }
    });
   ...
   //This works but not all the time. I'm setting the selected walker availability on the walker highlight page. 

  var walker_id =  Meteor.user().walkerId;
  Meteor.call('WalkerNameAboutRadius', walker_id, function (error, result) {
    if (result) {Session.set("WalkerNameAboutRadius", result)}
    if (error) { console.log(error)}
        console.log(result);
    console.log("Results**********************");
  });

   if (!Session.get('SelectedWalkerId')) {
    console.log("session was not already set");
                  Meteor.call ("MyWalkerId", function(error, results) {
                if (error) {
                    Router.go('/findWalker');
                    return sweetAlert({
                        title: "Uh Oh!",
                        type: "error",
                        text: "Please Select A Waliker"
                    });
                 } else {
                   Session.set('SelectedWalkerId', results);
                     return
                 }
            });
            } else {
                 var myWalker = Session.get('SelectedWalkerId');
            }
           

            Meteor.call("MyWalkersAvailability", myWalker, function (error, results) {
                if (error) {
                    return sweetAlert({
                        title: "Uh Oh!",
                        type: "error",
...
//  selectize modal stuff
 Meteor.call('IsPro', Meteor.user().walkerId, function (error, result) {
        if (result) {
            Session.set("IsPro", result);
            console.log("***is pro set *********");
        }
    });

    Meteor.setTimeout(function() {
        if (Session.get("IsPro") == true) {
            Meteor.call("GetProServicesList", Meteor.user().walkerId, function (error, results) {
                if (results) {
                    Session.set("ProServices", results);
                    console.log(results);
                    console.log("*******get pro services set******");
                }
                if (error) {
                    console.log(error);
                }
    ...
    Meteor.setTimeout(function() {
        ...
        $('#walkOptions').selectize({
            ...
            onItemAdd: function (value, $item) {
                if (Session.get("IsPro")) {
                    Meteor.call('ProServiceEventDetails', value, function (error, result) {
                        if (result) {
                            Session.set("ProServiceEventDetails", result); 
                            console.log("*******get pro services event details seet ******");
                        }
                    });
                }
            }
...
// A cmp function for determining the leftmost event
        ...
        $.extend(TimeGrid.prototype, {
            ...
            generateSegPositionCss: function(seg) {
                ...
                //Added by Josef Meile: holidays will be rendered with a
                //transparent color, they will occupy the whole table cell,
                //mouse events will be forwarded to the other events, and they won't overlap other events
                if (seg.event.holiday) {
                    left = 0;
                    right = 0;
                    props.width = "100%";

                    //Change the opacity if you want
                    props.opacity = "0.5";

                    //Need to be done this way; otherwise, you will get an error
                    //I guess is because of the '-', so JavaScript may think
                    //that you are  subtracting pointer and events.
                    props["pointer-events"] = "none";
                    if (seg.event.sendTop)
                        props.zIndex = 10;
                    else
                        props.zIndex = 0;
                }
                ...
                //Added by Josef Meile: This margins won't be needed by
                //the holidays, they will always occupy the whole table
                //cell
                if (!seg.event.holiday) {
                    if (shouldOverlap && seg.forwardPressure) {
                        // add padding to the edge so that forward stacked events don't cover the resizer's icon
...
// Append into an optionally-supplied `results` array and return.
        function computeSlotSegCollisions(seg, otherSegs, results) {
            ...
            for (var i = 0; i < otherSegs.length; i++) {
                ...
                //Added by Josef Meile: holiday won't collide with other events
...
// Responsible for managing width/height.
        ...
        /* A week view with an all-day cell area at the top, and a time grid below
         ----------------------------------------------------------------------------------------------------------------------*/
// a WeekView mixin for calculating dates and titles
    ...
    $('#calendar').fullCalendar({
        ...
        timeFormat: {
            agenda: ''    // was hh:mm TT{ - hh:mm TT}  but it looked ugly
        },
        eventColor: "#eb6a5a",
        /*defaultView: 'month',*/
        eventTextColor: '#fff',
        dayClick: function (date, jsEvent, view) {
            var thisView = $('#calendar').fullCalendar('getView');
            var thisFormatDate = date.format();
            if (thisView.name == 'month' || thisView.name == 'agendaWeek') {
                //$('#calendar').fullCalendar('changeView', 'agendaDay').fullCalendar('gotoDate', date); this aint working
                console.log(thisView.name);
                var day = date._d;
                $('#calendar').fullCalendar('changeView', "agendaDay");
                $('#calendar').fullCalendar('gotoDate', thisFormatDate);
                console.log(day);
            }
            if (thisView.name == 'agendaDay') {
                var modalTime = function (thisFormatDate) {
                    ...
                    var splitDate = thisFormatDate.toString().split("-");
                    console.log(splitDate);
                    var dayFix = splitDate[2].toString();
                    var dayFixed = dayFix.charAt(0)+dayFix.charAt(1);
                    var militarySelectedTime = dayFix.match(/[0-9][0-9][:][0-9][0-9][:]\w+/); //this is putting it to military time.
                    console.log(militarySelectedTime);
                    var selectedTime =  militaryToAmPm(militarySelectedTime.toString());
                    splitDate[2] = dayFixed;
                    console.log("From Modal Time:  "+ splitDate[1]+"/"+splitDate[2]+"/"+splitDate[0]);

                    return [splitDate[1]+"/"+splitDate[2]+"/"+splitDate[0], selectedTime];               
                };// end modal time
                Session.set("preSelectedDate", thisFormatDate.toString());
                var theSelectedDate = Session.get('preSelectedDate');

                console.log(theSelectedDate + " this is before modal time");
                var theResults = modalTime(thisFormatDate);
                console.log(theResults + " this is after modal time");
                //CalEvents.insert({title: 'New Event', start: date, end: date, actual_start: null, actual_end: null, eventDetails: 30});
                Session.set("selectedDate", theResults[0]);
                Session.set("selectedTime", theResults[1]);
                console.log(Session.get("selectedDate") + " " + Session.get("selectedTime"));
                $('#clientScheduleModal').modal('show');
        ...
        //     Session.set('editing_title', calEvent.title);
        //     Session.set('showEditEvent', true);
        //     Session.set('actual_start', null);
        //     Session.set('actual_end', null);
        //     $('#myModal').modal('show');
        // },
        events: function (start, end, timezone, callback) {
                ...
                Meteor.call("MyWalkersAvailability", myWalker, function (error, results) {
                 ...
                 var walkeravailEvents = Session.get('WalkerAvailEvents');
                console.log(walkeravailEvents);
                walkeravailEvents.forEach(function (evt) {
                                events.push({
                                    id:evt._id,
                                    title:evt.title,
                                    start:evt.start,
                ...
                var calEvents = Session.get("CalEventsForMyWalker")
               ...
               //  console.log("*******************************************************************");
               // console.log(Session.get("CalEventsForMyWalker"));
               // console.log("*******************************************************************");
        ...
        // testing refetch here  turn on autorun to make work again and remove refetch from dayclick
...
Template.clientCalendar.events({
    'click .schedule': function(evt) {
        evt.preventDefault();
        $('#clientScheduleModal').modal('show');

    },
    'click .fc-button-agendaWeek': function(e) {
    ...
    'click .fc-button-agendaDay': function(e) {
        $( ".fc-event-title" ).each(function( ) {
            if ( $( this).text() === "Available" ) {
                $( this).parent().addClass("Available");
            }
    ...
    'click .fc-button-month': function(e) {
        $( ".fc-event-title" ).each(function( ) {
            if ( $( this).text() === "Available" ) {
                $( this).parent().addClass("Available");
            }
    ...
    'click #scheduleModal': function (e,t) {
        if (Session.get("IsPro") == true) {
            Meteor.call("GetProServicesList", Meteor.user().walkerId, function (error, results) {
                if (results) {
                    Session.set("ProServices", results);
            ...
            onInitialize: function () {
                 Meteor.call('ProServiceEventDetails', function (error, result) {
                        if (result) {
                            Session.set("ProServiceEventDetails", result); 
                            console.log("*******get pro services event details seet ******");
                        }
...
Template.clientCalendar.helpers({
    lastMod: function(){
    ...
    name : function() {
        return Session.get("WalkerNameAboutRadius")[0].name
    },
    about_me: function () {
        return Session.get("WalkerNameAboutRadius")[0].about_me
    },
    walkRadius: function () {
        return Session.get("WalkerNameAboutRadius")[0].walkRadius
    },
    pic: function () {
        return Session.get("WalkerNameAboutRadius")[0].pic
    }
});

Template.editEvent4.helpers({
    evt: function() {
        var evt = CalEvents.findOne({_id:Session.get('editing_calevent')});
        return evt;
    }
});


Template.editEvent4.events({
    'click #save': function(evt, tmpl) {
        var eventTitle = $('#title').val();
        var eventDetails = $('#eventDetails').val();
        var eventId = Session.get('editing_calevent');
        var actualEnd = Session.get('actual_end');
        var actualStart = Session.get('actual_start');


        var updateCalEvent = function(id, title, eventDetails) {
            CalEvents.update(id,
                {$set: {title:title}},
                {$set: {eventDetails:eventDetails}}
            );
            //CalEvents.update(id, {$set: {eventDetails:eventDetails}});
        };


        updateCalEvent(eventId, eventTitle, eventDetails);
    ...
    'click #remove': function(evt, tmpl) {
        ...
        var id = Session.get('editing_calevent');
        deleteCalEvent(id);
        //$('#myModal').modal('hide');
    }
});
