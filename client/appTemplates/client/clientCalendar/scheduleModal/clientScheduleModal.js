...
Template.clientScheduleModal.rendered = function() {
...
Template.clientScheduleModal.helpers({
    ...
    pro: function () {
        return Session.get("IsPro");
    },
    proService: function () {
        var proServices = Session.get("ProServices");
        console.log(proServices + "  from the pro service helper.");
        var proServicesArray = [];

        for (var i=0; i<proServices.length; i++) {
            proServicesArray.push({
                "title": proServices[i].title,
                "length": proServices[i].length,
                "price": proServices[i].price,
                "service": proServices[i].service,
                "id": proServices[i]._id
                
            });
        }
        return proServicesArray;
    }
});
...
Template.clientScheduleModal.events({
    'click #closeRecurring': function (evt, templ) {
        $('#calendar').fullCalendar('refetchEvents');
    },
    'click .selectize-dropdown': function (e,t) {
        alert("clicked");
        Meteor.setTimeout(function() {
            alert("selected " + $('.selected').attr("data-value"));
        },1000);
    },
    'click #scheduleWalks': function(evt) {
         if (Session.get("IsPro")) {
            Meteor.call("ProServiceEventDetails", $('#walkOptions').val(), function (e,r){
                if (r) {
                    Session.set("ProServiceEventDetails",r);
                }
            });
        }
        ...
        console.log(preEventStart, "*********preEventStart*************");
        ...
        console.log(preEventTime, "*********preEventStart*************");
        var eventTime = militaryTime(preEventTime);
        var eventDuration = $('#walkOptions').val();
        if (Session.get("IsPro")) {
            eventDuration = Session.get("ProServiceEventDetails").length;
            console.log(eventDuration);
            console.log(" *********event duration*********for pro****")
        }
       ...
       //starting availability check
        ...
        console.log(eventEndMinutes + " ******************for pro****")
        ...
        //*******************************************THIS IS TO NOT ALLOW SCHEDULINGN EVENTS IN THE PAST*****IT DOESNT WORK****
        // if (d.getDate() > arrayOfDayOnly[1]) {
        ...
        if (eventDuration === "") {
            ...
            Meteor.setTimeout(function() {
                if (Session.get('isWalkerAvailable') === true && Session.get('isThereAConflict') === false) {
                    ...
                    //testing pro add 
                    if (Session.get("IsPro")) {
                        console.log("Yep, this is a pro");

                        title = Session.get("ProServiceEventDetails").title;
                        eventDuration = Session.get("ProServiceEventDetails").length;
                        eventEndMinutes = eventStartMinutes + Number(eventDuration);
                        amount = Session.get("ProServiceEventDetails").price;
                        
                        console.log(title,eventDuration,eventEndMinutes,amount);
                        //try adding event
                        Meteor.call('AddProWalk',
                        amount,
                        title,
            ...
            Meteor.setTimeout(function() {
                console.log(Session.get('isWalkerAvailable') === true && Session.get('isThereAConflict') === false);
                console.log(Session.get('isWalkerAvailable7') === true && Session.get('isThereAConflict7') === false);
                console.log(Session.get('isWalkerAvailable14') === true && Session.get('isThereAConflict14') === false);
                console.log(Session.get('isWalkerAvailable21') === true && Session.get('isThereAConflict21') === false);
                console.log(Session.get('isWalkerAvailable28') === true && Session.get('isThereAConflict28') === false);
                console.log(Session.get('isWalkerAvailable35') === true && Session.get('isThereAConflict35') === false);
                console.log(Session.get('isWalkerAvailable42') === true && Session.get('isThereAConflict42') === false);
                console.log(Session.get('isWalkerAvailable49') === true && Session.get('isThereAConflict49') === false);
